		...
		where T : class, ICmObject
	{
		...
		public virtual void Replace(int start, int numberToDelete, IEnumerable<ICmObject> thingsToAdd)
		{
			...
			if (deletedSubset.Count() > 0)
			{
				...
				Debug.Assert(removedAndReAdded.Count + removed.Count() == numberToDelete,
					"Logic for handling deletes is incomplete, we won't be attempting enough removes.");
				...
				using (var itr = thingsToAdd.GetEnumerator())
				{
					...
					foreach (var removedObject in removedItemsNeedingSideEffects)
					{
						...
						RemoveRefsTo((ICmObjectInternal) deadObject);
						// Before side effects (e.g., target needs to know it is no longer referring).
